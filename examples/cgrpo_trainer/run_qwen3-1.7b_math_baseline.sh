#!/bin/bash
# Baseline GRPO on MATH numeric 3k
...
export VERL_FILE_LOGGER_PATH=logs/math_baseline_metrics.jsonl
mkdir -p logs

TRAIN_DATA="${TRAIN_DATA:-/root/autodl-tmp/cgrpo-verl/data/math/math_numeric_processed_3k.jsonl}"
...
python3 -m verl.trainer.main_cgrpo \
    ...
    trainer.project_name='math_baseline' \
    trainer.experiment_name='math_baseline_3k_0404' \
    trainer.default_local_dir=checkpoints/math_baseline_3k \
    ...
    trainer.debug_dump_freq=10 \
    trainer.debug_dump_dir=debug_samples/math_baseline \
