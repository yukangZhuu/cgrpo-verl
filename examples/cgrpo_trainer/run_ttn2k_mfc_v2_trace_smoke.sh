#!/bin/bash
# Smoke test for MFC v2 — pipeline verification only.
# Mirrors run_ttn2k_mfc_trace_smoke.sh but with mfc_curriculum.variant=v2 and tau=0.5.
# Produces one JSONL line per step in adaptive_trace/adaptive_train_trace.jsonl
# so rho_max trajectories and confirmation events can be inspected post-run.
set -x
...
python3 -m verl.trainer.main_cgrpo \
    ...
    trainer.experiment_name='mfc_v2_trace_smoke' \
    trainer.default_local_dir=checkpoints/mfc_v2_trace_smoke \
    \
