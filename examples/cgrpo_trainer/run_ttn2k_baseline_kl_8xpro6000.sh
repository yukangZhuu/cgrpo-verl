#!/bin/bash
# B1-KL: Baseline GRPO on TTN-2K train (2,000 olympiad problems)
# No teacher guidance — guidance_mode=none
# KL loss enabled (use_kl_loss=True)
...
export VERL_FILE_LOGGER_PATH=logs/ttn2k_baseline_kl_8xpro6000_metrics.jsonl
mkdir -p logs
...
python3 -m verl.trainer.main_cgrpo \
    ...
    trainer.experiment_name='ttn2k_baseline_kl_8xpro6000' \
    trainer.default_local_dir=checkpoints/ttn2k_baseline_kl \
    ...
    trainer.debug_dump_dir=debug_samples/ttn2k_baseline_kl \
