...
# Qwen3-0.6B as base, 8x RTX PRO 6000 (96 GB).
#
# Based directly on run_ttn2k_unsolvable_mfc_v2_hint_8xpro6000.sh (Qwen3-1.7B)
# with VRAM-constrained knobs relaxed for 0.6B on PRO6000 (see dapo baseline
# launcher in this folder for the rationale of every delta).
#
# Algorithm hyperparameters (mfc_curriculum.variant / tau / default_rho_max
# and all KL/clip/lr) are byte-identical to all other MFC v2 launchers so
# cross-base-size AND cross-hardware comparison is direct.
#
...
export VERL_FILE_LOGGER_PATH=logs/ttn2k_unsolvable_mfc_v2_hint_qwen3_0.6b_8xpro6000_metrics.jsonl
mkdir -p logs
...
python3 -m verl.trainer.main_cgrpo \
    ...
    trainer.experiment_name='ttn2k_unsolvable_mfc_v2_hint_qwen3_0.6b_8xpro6000' \
    trainer.default_local_dir=checkpoints/ttn2k_unsolvable_mfc_v2_hint_qwen3_0.6b_pro6000 \
    ...
    trainer.debug_dump_dir=debug_samples/ttn2k_unsolvable_mfc_v2_hint_qwen3_0.6b_pro6000 \
