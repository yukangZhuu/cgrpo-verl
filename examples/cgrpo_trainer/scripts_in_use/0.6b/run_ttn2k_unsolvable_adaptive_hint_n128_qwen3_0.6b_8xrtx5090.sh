...
# with Qwen3-0.6B as base, 8x RTX 5090 (32 GB).
#
# Direct port of run_ttn2k_unsolvable_adaptive_hint_n128_8xrtx5090.sh
# (Qwen3-1.7B / 8x RTX 5090).  Qwen3-0.6B is roughly 35% the size of 1.7B,
# so the per-GPU memory pressure is materially lower; we therefore relax the
# 5090 / 1.7B safety knobs back toward the original PRO6000 settings:
#
#     rollout.gpu_memory_utilization      0.55 (1.7B / 5090) -> 0.65
#     actor.ppo_max_token_len_per_gpu     24000             -> 32000
#     rollout.max_num_batched_tokens      16384             -> 24576
#     rollout.log_prob_micro_batch_size_per_gpu  8          -> 16
#     ref.log_prob_micro_batch_size_per_gpu      8          -> 16
#
# Everything else (batch=128, rollout.n=8, response_length=8192, lr=1e-6,
# KL/clip, AdaBack tau/p_zero/default_rho/min_step_delta) is held identical
# to the 1.7B / PRO6000 + 1.7B / 5090 launchers so cross-base-size results
...
export VERL_FILE_LOGGER_PATH=logs/ttn2k_unsolvable_adaptive_hint_n128_qwen3_0.6b_8xrtx5090_metrics.jsonl
mkdir -p logs
...
python3 -m verl.trainer.main_cgrpo \
    ...
    trainer.experiment_name='ttn2k_unsolvable_adaptive_hint_n128_qwen3_0.6b_8xrtx5090' \
    trainer.default_local_dir=checkpoints/ttn2k_unsolvable_adaptive_hint_n128_qwen3_0.6b_5090 \
    ...
    trainer.debug_dump_dir=debug_samples/ttn2k_unsolvable_adaptive_hint_n128_qwen3_0.6b_5090 \
