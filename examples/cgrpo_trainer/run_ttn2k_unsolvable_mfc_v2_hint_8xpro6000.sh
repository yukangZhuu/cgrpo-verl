#!/bin/bash
# MFC v2 (minimal monotone variant) on the pure-128 unsolvable pool.
# Dataset: ttn_unsolvable_pass64_n128/dataset.jsonl (no anchors).
#
# v2 strips MFC down to the bare structural skeleton: per-sample
# rho_max ∈ [0,1], rho ~ Uniform(0, rho_max), monotone non-increasing
# update on `avg_reward >= tau`.  No safety valve, no exploit/probe split.
# Single hyperparameter: tau.
#
# Designed to be 1:1 comparable with:
#   - run_ttn2k_unsolvable_mfc_hint_8xpro6000.sh         (MFC v1)
#   - run_ttn2k_unsolvable_adaptive_hint_n128_8xpro6000  (AdaBack baseline)
# All training knobs identical; only mfc_curriculum.* + experiment names differ.
#
...
export VERL_FILE_LOGGER_PATH=logs/ttn2k_unsolvable_mfc_v2_hint_8xpro6000_metrics.jsonl
mkdir -p logs
...
python3 -m verl.trainer.main_cgrpo \
    ...
    trainer.experiment_name='ttn2k_unsolvable_mfc_v2_hint_8xpro6000' \
    trainer.default_local_dir=checkpoints/ttn2k_unsolvable_mfc_v2_hint \
    ...
    trainer.debug_dump_dir=debug_samples/ttn2k_unsolvable_mfc_v2_hint \
